  ...
  return (
    <div className="w-full max-w-md bg-app-card p-8 rounded-xl shadow-soft">
      ...
      <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
        <input className="w-full bg-slate-800 rounded-lg p-3 border border-slate-700" placeholder="Name" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
        ...
        <button disabled={loading} type="submit" className="w-full bg-app-primary rounded-lg p-3 font-semibold disabled:opacity-70">{loading ? "Creating..." : "Register"}</button>
      </form>
      <p className="text-app-muted text-sm mt-4">
        Already have an account? <Link className="text-app-primary" to="/login">Login</Link>
      </p>
    </div>
  );
}
